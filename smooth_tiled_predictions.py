...
def _windowed_subdivs(padded_img, window_size, subdivisions, nb_classes, pred_func):
    ...
    batch_size = 4  # ou 2 si ta VRAM est trop limitée
    results = []
    for i in range(0, len(subdivs), batch_size):
        batch = subdivs[i:i+batch_size]
        results.append(pred_func(batch))
    subdivs = np.concatenate(results, axis=0)
